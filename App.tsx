  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl p-6 text-white shadow-lg">
        ...
        <p className="opacity-90">Ready to learn something new today?</p>
      </div>

      <div>
        <h3 className="font-bold text-gray-800 mb-3 text-lg">My Courses</h3>
        {courses?.map(course => (
          <CourseCard key={course.id} course={course} onClick={() => navigate(`/course/${course.id}`)} />
        ))}
        {(!courses || courses.length === 0) && <p className="text-center text-gray-500 py-4">Loading courses...</p>}
      </div>
    </div>
  );
};
...
const CourseView: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => navigate('/')} className="text-sm text-blue-600 mb-2 font-medium">← Back to Home</button>
      <div className="flex justify-between items-start">
         <h2 className="text-2xl font-bold text-gray-800">{course.title}</h2>
         {course.isDownloaded && <span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-medium">Available Offline</span>}
      </div>
             ...
             <div className="flex items-center space-x-3">
               ...
               <div>
                 ...
                 <p className="text-xs text-gray-500 flex items-center gap-1">
                    ...
                    {mod.isCompleted ? 'Completed' : mod.type === 'quiz' ? 'Take Quiz' : 'Watch Video'}
                 </p>
               </div>
             </div>
           ...
           <>
              {moduleData.type === 'quiz' || showQuiz ? (
                 <div className="bg-white border-2 border-blue-100 p-4 rounded-xl">
                    <h4 className="font-bold mb-3 text-lg">Quiz Time!</h4>
                 ...
                 <button onClick={() => moduleData.quiz ? setShowQuiz(true) : handleFinish()} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold text-lg shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2">
                   ...
                   {moduleData.quiz ? "Start Quiz" : "Mark as Completed"}
                 </button>
              )}
           </>
         )}
...
const App: React.FC = () => {
  ...
  if (checkingAuth) {
    return <div className="min-h-screen flex items-center justify-center bg-blue-50 text-blue-600">Loading Padho India...</div>;
  }
  ...
  if (isLoggingOut) {
     return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-blue-50 text-blue-600">
           ...
           <p className="text-lg font-bold animate-pulse">Logging out securely...</p>
        </div>
     );
  }

  if (!isLoggedIn) {
    return <AuthGateway onLoginSuccess={handleLogin} />;
  }
